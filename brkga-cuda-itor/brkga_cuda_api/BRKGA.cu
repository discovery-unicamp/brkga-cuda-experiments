...
BRKGA::BRKGA(unsigned n, ConfigFile &conf_file) {
  ...
  test_memory_malloc(
      cudaMalloc((void **)&d_scores, number_chromosomes * sizeof(float)), 3,
      total_memory);
  ...
  h_chromosome_gene_idx = (ChromosomeGeneIdxPair *)malloc(
      number_chromosomes * chromosome_size * sizeof(ChromosomeGeneIdxPair));
  test_memory_malloc(cudaMalloc((void **)&d_chromosome_gene_idx,
                                number_chromosomes * chromosome_size *
                                    sizeof(ChromosomeGeneIdxPair)),
                     5, total_memory);

  total_memory += number_chromosomes * sizeof(float);
  test_memory_malloc(cudaMalloc((void **)&d_random_elite_parent,
                                number_chromosomes * sizeof(float)),
                     6, total_memory);

  total_memory += number_chromosomes * sizeof(float);
  test_memory_malloc(
      cudaMalloc((void **)&d_random_parent, number_chromosomes * sizeof(float)),
      7, total_memory);
  ...
  test_memory_malloc(
      cudaMalloc((void **)&d_best_solutions,
  ...
  if ((n * population_size * number_populations) % THREADS_PER_BLOCK == 0)
    this->dimGrid.x =
        (n * population_size * number_populations) / THREADS_PER_BLOCK;
  else
    this->dimGrid.x =
        (n * population_size * number_populations) / THREADS_PER_BLOCK + 1;
  this->dimGrid.y = 1;
  this->dimGrid.z = 1;

  // Grid dimension when having one thread per chromosome
  if ((population_size * number_populations) % THREADS_PER_BLOCK == 0)
    this->dimGridChromo.x =
        (population_size * number_populations) / THREADS_PER_BLOCK;
  else
    this->dimGridChromo.x =
        (population_size * number_populations) / THREADS_PER_BLOCK + 1;
  this->dimGridChromo.y = 1;
  this->dimGridChromo.z = 1;

  // Grid dimension when having one block to process each chromosome
  this->dimGrid_population.x =
      population_size *
      number_populations; // one block to process each chromosome
...
void BRKGA::test_memory_malloc(cudaError_t err, unsigned code,
...
/***
        ...
        It also receives the number of elements (num) in the array info and the
size (size) of each element. Notice we assume the type of the info elements to
be float.
***/
void BRKGA::setInstanceInfo(void *info, long unsigned num, long unsigned size) {
  if (info != NULL) {
    ...
    if (decode_type == DEVICE_DECODE ||
      ...
      test_memory_malloc(cudaMalloc((void **)&d_instance_info, num * size), 8,
                         total_memory);
...
/***
        If HOST_DECODE is used then this function decodes each cromosome with
...
#pragma omp parallel for default(none)                                         \
  ...
  for (int b = 0; b < dimGrid.x; b++) {
    for (int t = 0; t < dimBlock.x; t++) {
      unsigned tx = b * dimBlock.x + t; // Found the thread index since each
  ...
  cudaMemcpy(d_scores, h_scores, number_chromosomes * sizeof(float),
             cudaMemcpyHostToDevice);
}

/***
        If DEVICE_DECODE is used then this kernel function decodes each
cromosome with the device_decode function provided in Decoder.cpp. We use one
thread per cromosome to process them.
***/
...
/***
        If DEVICE_DECODE is used then this function decodes each cromosome with
...
/***
        If DEVICE_DECODE_CHROMOSOME_SORTED is used then this kernel function
decodes each cromosome with the device_decode_chromosomeSorted function provided
...
index of each gene in the cromosome.
***/
__global__ void
...
/***
        If DEVICE_DECODE_CHROMOSOME_SORTED is used then this function decodes
...
/***
 ...
 Kernel function used to save for each gene of each chromosome, the chromosome
...
/***
 ...
 After sorting by gene we need to reagroup genes by their chromosomes which are
indexed by threadIdx.
***/
...
chromosomes by their values, and than we sort by the chromosome index, and since
...
/***
Kernel function, where each thread process one chromosome. It receives the
current population *d_population, the next population pointer *d_population2,
two random vectors for indices of parents, d_random_elite_parent and
d_random_parent,
***/
__global__ void
device_next_population(float *d_population, float *d_population2,
  ...
  if (inside_pop_idx < elite_size) {
    ...
    // mutants_size thread is responsible to crossover
...
/***
Main function of the BRKGA algorithm. It evolves K populations for a certain
...
void BRKGA::evolve(int number_generations) {
  ...
  // After this call the vector d_scores_idx has all threads sorted by
  // population, and inside each population, threads are sorted by score
  sort_chromosomes();

  // This call initialize the whole area of the next population d_population2
  // with random values. So mutantes are already build. For the non mutants we
  // use the random values generated here to perform the crossover on the current
...
/***
Kernel function that sets for each cromosome its global index (among all
...
sorts: first we sort by the chromosome scores, and than by their population
...
chromosome of its own population into the other populations. To do: make kernel
save in local memory the chromosome and then copy to each other population
***/
