...
BRKGA::BRKGA(unsigned n, ConfigFile &conf_file, bool evolve_coalesced,
  ...
  this->n_pop_pipe = min(n_pop_pipe, number_populations);
  if (evolve_pipeline)
...
#pragma omp task depend(out : f1[p])
          {
            ...
            // omp_get_thread_num());
            if (p < n_pop_pipe) {
              if (decode_type == DEVICE_DECODE_CHROMOSOME_SORTED) {
                evaluate_chromosomes_sorted_device_pipe(p);
              } else if (decode_type == DEVICE_DECODE) {
                evaluate_chromosomes_device_pipe(p);
              } else {
                std::cout << "No decode in device specified! Using deconding "
                             "on host instead!"
                          << std::endl;
...
#pragma omp task depend(in : f1[p])
          {
            ...
            device_next_population_coalesced_pipe<<<dimGrid_gene_pipe, dimBlock,
                ...
                chromosome_size, population_size, elite_size, mutants_size,
                rhoe, d_scores_idx_pipe[p], num_genes);
