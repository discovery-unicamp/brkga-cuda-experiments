...
BRKGA::BRKGA(unsigned n, unsigned p, float pe, float pm, float rhoe, unsigned K, unsigned decode_type, unsigned NUM_THREADS, unsigned RAND_SEED){
	...
	//set to the maximum number of streams allowed in CUDA Fermi
	if(K > MAX_STREAMS){
		K = MAX_STREAMS;
	}
	...
	this->dimGrid.x = population_size/THREADS_PER_BLOCK; //used when processing one population
	this->dimGrid2.x = (population_size*number_populations)/THREADS_PER_BLOCK; //used when processing all populations
...
void BRKGA::reset_population(void){
	curandGenerateUniform(rand_gen, d_population, number_chromosomes*chromosome_size);
}
...
***/
void BRKGA::evaluate_chromosomes_host(unsigned p){
	cudaMemcpy(h_population, d_population, number_chromosomes*chromosome_size*sizeof(float),cudaMemcpyDeviceToHost);

	#pragma omp parallel for default(none) shared(dimGrid,dimBlock,h_population,h_scores,p) collapse(2) num_threads(NUM_THREADS)
		for(int b=0; b < dimGrid.x ; b++){
			for(int t=0; t < dimBlock.x; t++){
				unsigned tx = b*dimBlock.x + t; //Found the thread index since each thread is associated with
			                                //a cromosome.
				float *chromosome = h_population + (p*population_size*chromosome_size + tx*chromosome_size);
...
***/
__global__ 
void decode(float *d_scores, float *d_population, int chromosome_size, void * d_instance_info, unsigned pop_idx, unsigned population_size){
	unsigned global_tx = pop_idx*population_size + blockIdx.x*blockDim.x + threadIdx.x;	
...
void BRKGA::evaluate_chromosomes_device(){
	...
	for(int p=0; p<number_populations; p++)
		decode<<<dimGrid, dimBlock, 0, streams[p]>>>(d_scores, d_population2, chromosome_size, d_instance_info, p, population_size);
}
...
void decode_chromosomes_sorted(float *d_scores, ChromosomeGeneIdxPair *d_chromosome_gene_idx, int chromosome_size, void *d_instance_info,
		unsigned pop_idx, unsigned population_size){
	unsigned global_tx = pop_idx*population_size + blockIdx.x*blockDim.x + threadIdx.x;	
...
void BRKGA::evaluate_chromosomes_sorted_device(){
	...
	for(int p=0; p<number_populations; p++)
		decode_chromosomes_sorted<<<dimGrid, dimBlock, 0, streams[p]>>>(d_scores, d_chromosome_gene_idx, chromosome_size, d_instance_info, p, population_size);
}
...
void BRKGA::sort_chromosomes_genes(){
	...
	device_set_chromosome_gene_idx<<<dimGrid2, dimBlock>>>(d_chromosome_gene_idx, chromosome_size);
...
void BRKGA::evolve(int number_generations){
	...
	curandGenerateUniform(rand_gen, d_random_elite_parent, number_chromosomes);
	curandGenerateUniform(rand_gen, d_random_parent, number_chromosomes);
	...
	for(int p=0; p<number_populations; p++){
		...
		device_next_population<<<dimGrid, dimBlock, 0, streams[p]>>>(d_population, d_population2,  d_random_elite_parent,
...
void BRKGA::initialize_population(int p){
	if(p==1)
		curandGenerateUniform(rand_gen, d_population, number_chromosomes*chromosome_size);
	if(p==2)
		curandGenerateUniform(rand_gen, d_population2, number_chromosomes*chromosome_size);
}
...
***/
__global__ 
void device_set_idx(PopIdxThreadIdxPair *d_scores_idx, unsigned population_size, unsigned pop_idx){
	int tx = pop_idx*population_size + blockIdx.x*blockDim.x + threadIdx.x;
//	d_scores_idx[tx].popIdx = tx/population_size; 
	d_scores_idx[tx].popIdx = pop_idx;
...
***/
void BRKGA::sort_chromosomes(unsigned p){
	//for(int p=0; p<number_populations; p++){
		//For each thread we store in d_scores_idx the global chromosome index and its population index.
		device_set_idx<<<dimGrid, dimBlock,0,streams[p]>>>(d_scores_idx, population_size,p);
		...
		thrust::stable_sort_by_key(thrust::cuda::par.on(streams[p]), keys+p*(population_size), keys + (p+1)*population_size, vals+p*(population_size));
		...
		thrust::stable_sort_by_key(thrust::cuda::par.on(streams[p]), vals+p*(population_size), vals + (p+1)*population_size, keys+p*(population_size));
	//}
...
***/
__global__ 
void device_set_idx2(PopIdxThreadIdxPair *d_scores_idx, unsigned population_size){
	int tx = blockIdx.x*blockDim.x + threadIdx.x;
	d_scores_idx[tx].popIdx = tx/population_size; 
	d_scores_idx[tx].thIdx = tx; 
}
...
void BRKGA::global_sort_chromosomes(){
	...
	fprintf(stderr, "aqui 44\n");
	device_set_idx2<<<dimGrid2, dimBlock>>>(d_scores_idx, population_size);
	thrust::device_ptr<float> keys(d_scores);
	thrust::device_ptr<PopIdxThreadIdxPair> vals(d_scores_idx);
