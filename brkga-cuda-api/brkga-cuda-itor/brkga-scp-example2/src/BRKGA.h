...
private:
	...
	curandGenerator_t rand_gen; //cuda ramdom number generator
	dim3 dimBlock;
  dim3 dimGrid;
  dim3 dimGrid2;
	cudaStream_t streams[MAX_STREAMS]; //maximum number of streams of Fermi
  ...
  void initialize_population(int p);
	...
	void sort_chromosomes(unsigned p);
	void sort_chromosomes_genes();
	void evaluate_chromosomes_host(unsigned p);
