...
BRKGA::BRKGA(unsigned n, unsigned p, float pe, float pm, float rhoe, unsigned K, unsigned decode_type, unsigned NUM_THREADS, unsigned RAND_SEED){
	...
	//set the number of populations to the maximum number of streams in cuda
	if(K > MAX_STREAM){
		K = MAX_STREAM;
	}

	population_size = p;
	number_populations = K;
	number_chromosomes = p * K; 
	chromosome_size = n;
	elite_size = (unsigned)(pe*p);
	//if(elite_size % 8 != 0)//elite_size is a multiple of 8
	//	elite_size = ((elite_size/8)+1)*8;
	mutants_size = (unsigned)(pm*p);
	//the size of the population of normal chromossomes must be a multiple of elite_size to efficiently perform crossover using shared memory
	normal_size = population_size-elite_size-mutants_size;
	//if(normal_size%elite_size != 0){
	//	normal_size = ((normal_size/elite_size)+1)*elite_size;
	//	if(population_size-elite_size-normal_size<=0){
	//		normal_size = ((normal_size/elite_size))*elite_size;			
	//	}
	//	mutants_size = population_size-elite_size-normal_size;
	//}
	this->rhoe = rhoe;
	...
	if(elite_size + mutants_size + normal_size > population_size) { throw range_error("elite + mutant + normal sets greater than population size (p)."); }
	if(normal_size <= 0) { throw range_error("normal set size is 0."); }
...
__global__ 
void device_next_crossover(float *d_population, float *d_population2, 
	float *d_random_elite_parent, float *d_random_parent, int chromosome_size, 
	unsigned population_size, unsigned elite_size, unsigned mutants_size, float rhoe, PopIdxThreadIdxPair *d_scores_idx){

	unsigned tx = blockIdx.x*population_size + threadIdx.x + elite_size; //global thread index
	unsigned chromosome_idx = tx*chromosome_size;
	unsigned pop_idx = (unsigned)tx/population_size; //the population index of this thread
	unsigned inside_pop_idx = tx%population_size; 
	//below are the inside population random indexes of a elite parent and regular parent for crossover
...
void device_next_crossover(float *d_population, float *d_population2, 
	float *d_random_elite_parent, float *d_random_parent, int chromosome_size, 
	unsigned population_size, unsigned elite_size, unsigned mutants_size, float rhoe, PopIdxThreadIdxPair *d_scores_idx, unsigned pop_idx){

	//unsigned tx = blockIdx.x*population_size + threadIdx.x + elite_size; //global thread index
	unsigned tx = pop_idx*population_size + blockIdx.x*blockDim.x + threadIdx.x; //global thread index
	unsigned chromosome_idx = tx*chromosome_size;
	//unsigned pop_idx = (unsigned)tx/population_size; //the population index of this thread
	unsigned inside_pop_idx = tx%population_size; 
	//below are the inside population random indexes of a elite parent and regular parent for crossover
...
void device_next_crossover(float *d_population, float *d_population2, 
	...
	//implicity assume that blockDim.x == normal_size/elite_size < 100
	__shared__ float elite[100];

	//pop_idx is the population index
	unsigned tx = pop_idx*population_size + blockIdx.x*blockDim.x + threadIdx.x; //global thread index
	unsigned chromosome_idx = tx*chromosome_size;
	//below are the inside population indexes of a elite parent and a random regular parent for crossover
	unsigned parent_elite_idx = blockIdx.x;
	unsigned parent_idx = (unsigned)(elite_size+ceilf(d_random_parent[tx]*(population_size-elite_size))-1);
	...
	int k;
	for(k=0; k<chromosome_size/blockDim.x; k++){
		elite[threadIdx.x] = d_population[elite_chromosome_idx + k*blockDim.x + threadIdx.x];
		__syncthreads();

		for(int i=0; i<blockDim.x; i++){
			if(d_population2[chromosome_idx + k*blockDim.x + i] <= rhoe)
				//copy allele from elite parent
				d_population2[chromosome_idx + k*blockDim.x + i] = elite[i];
			else
				//copy allele from regular parent
				d_population2[chromosome_idx + k*blockDim.x + i] = d_population[parent_chromosome_idx + k*blockDim.x + i];
		}
	}
	if(k*blockDim.x + threadIdx.x < chromosome_size)
		elite[threadIdx.x] = d_population[elite_chromosome_idx + k*blockDim.x + threadIdx.x];
	__syncthreads();
	
	for(int i=0; i<(chromosome_size%blockDim.x); i++){
		if(d_population2[chromosome_idx + k*blockDim.x + i] <= rhoe)
			//copy allele from elite parent
			d_population2[chromosome_idx + k*blockDim.x + i] = elite[i];
		else
...
***/
__global__ 
void device_next_elite(float *d_population, float *d_population2, int chromosome_size, 
	unsigned population_size, PopIdxThreadIdxPair *d_scores_idx){

	unsigned tx = blockIdx.x*population_size + threadIdx.x; //global elite thread index
	unsigned chromosome_idx = tx*chromosome_size;
	unsigned elite_chromosome_idx = d_scores_idx[tx].thIdx*chromosome_size;
...
void BRKGA::evolve(int number_generations){
	...
	//Kernel function, where each thread process one elite chromosome of the next population.
	device_next_elite<<<number_populations, elite_size, 0, stream[0]>>>(d_population, d_population2, chromosome_size,
	  population_size, d_scores_idx);

	device_next_crossover<<<dimGrid, dimBlock, 0, stream[1]>>>(d_population, d_population2,  d_random_elite_parent,
		d_random_parent, chromosome_size, population_size, elite_size, mutants_size, rhoe, d_scores_idx);

	/*for(int p=0; p<number_populations; ++p){
		//Kernel function, where each thread process one crossover chromosome of the next population.
		device_next_crossover<<<population_size/THREADS_PER_BLOCK, THREADS_PER_BLOCK, 0, stream[p]>>>(d_population, d_population2,  d_random_elite_parent,
			d_random_parent, chromosome_size, population_size, elite_size, mutants_size, rhoe, d_scores_idx, p);

		//device_next_crossover<<<elite_size, normal_size/elite_size, 0, stream[p]>>>(d_population, d_population2,  d_random_elite_parent,
		//	d_random_parent, chromosome_size, population_size, elite_size, mutants_size, rhoe, d_scores_idx, p);
	}*/
