...
__global__ void gpuInit(int n, float* d_pop, int* d_val, curandState* d_crossStates) {
  ...
  for (; idx < bidx + n; idx += offset) d_pop[idx] = RNG_real(curand(&d_crossStates[auxidx]));

  if (!threadIdx.x) d_val[blockIdx.x] = blockIdx.x;
}
...
__global__ void offspring(float* d_current,
  ...
  if (!threadIdx.x) d_nextFitValues[blockIdx.x] = blockIdx.x;
}
