...
void GPUBRKGA<Decoder>::cpyHost() {
  ...
  cudaCheck(cudaMemcpy(h_populations, d_current, K * p * n * sizeof(float), cudaMemcpyDeviceToHost));
  cudaCheck(cudaMemcpy(h_fitnessKeys, d_currFitnessKeys, K * p * sizeof(float), cudaMemcpyDeviceToHost));
  cudaCheck(cudaMemcpy(h_fitnessValues, d_currFitnessValues, K * p * sizeof(int), cudaMemcpyDeviceToHost));
  debugCudaSync;
}

template <class Decoder>
GPUBRKGA<Decoder>::GPUBRKGA(unsigned _n,
                            unsigned _p,
                            double _pe,
                            double _pm,
                            double _rhoe,
                            const Decoder& _refDecoder,
                            int _seed,
                            bool _gpu_deco,
  ...
  // Device data alloc
  cudaCheck(cudaMalloc((void**)&d_current, (K * p * n) * sizeof(float)));
  cudaCheck(cudaMalloc((void**)&d_previous, (K * p * n) * sizeof(float)));

  cudaCheck(cudaMalloc((void**)&d_currFitnessKeys, (K * p) * sizeof(float)));
  cudaCheck(cudaMalloc((void**)&d_currFitnessValues, (K * p) * sizeof(int)));
  cudaCheck(cudaMalloc((void**)&d_prevFitnessKeys, (K * p) * sizeof(float)));
  cudaCheck(cudaMalloc((void**)&d_prevFitnessValues, (K * p) * sizeof(int)));

  // RNG states
  cudaCheck(cudaMalloc((void**)&d_crossStates, (p * thr) * sizeof(curandState)));
  cudaCheck(cudaMalloc((void**)&d_mateStates, 2 * p * sizeof(curandState)));
  debugCudaSync;
...
GPUBRKGA<Decoder>::~GPUBRKGA() {
  cudaCheck(cudaFree(d_current));
  cudaCheck(cudaFree(d_previous));
  cudaCheck(cudaFree(d_currFitnessKeys));
  cudaCheck(cudaFree(d_currFitnessValues));
  cudaCheck(cudaFree(d_prevFitnessKeys));
  cudaCheck(cudaFree(d_prevFitnessValues));
  cudaCheck(cudaFree(d_crossStates));
  cudaCheck(cudaFree(d_mateStates));
  debugCudaSync;
...
Individual GPUBRKGA<Decoder>::getBestIndividual() {
  ...
  cudaCheck(cudaMalloc((void**)&d_bk, sizeof(int)));
  debugCudaSync;
  bestK<<<1, 1>>>(d_currFitnessKeys, K, p, d_bk);
  debugCudaSync;
  cudaCheck(cudaMemcpy(&h_bk, d_bk, sizeof(float), cudaMemcpyDeviceToHost));
  debugCudaSync;
  float* al = (float*)malloc(n * sizeof(float));
  std::pair<float, int> ft;

  cudaCheck(cudaMemcpy(&ft.first, d_currFitnessKeys + getOffset(h_bk, false), sizeof(float), cudaMemcpyDeviceToHost));
  cudaCheck(cudaMemcpy(&ft.second, d_currFitnessValues + getOffset(h_bk, false), sizeof(int), cudaMemcpyDeviceToHost));
  cudaCheck(cudaMemcpy(al, d_current + getInd(h_bk, ft.second, true), n * sizeof(float), cudaMemcpyDeviceToHost));
  debugCudaSync;
...
void GPUBRKGA<Decoder>::exchangeElite(unsigned M) throw(std::range_error) {
  ...
  for (int i = 0; i < K; i++) {
    if (d_temp_storage == NULL) {
      cudaCheck(cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_currFitnessKeys + (i * p),
                                                d_prevFitnessKeys + (i * p), d_currFitnessValues + (i * p),
                                                d_prevFitnessValues + (i * p), p));
...
inline void GPUBRKGA<Decoder>::initializeGPU() {
  ...
  for (int i = 0; i < K; i++) {
    ...
    if (gpu_deco) {
      refDecoder.Decode(d_current, d_currFitnessKeys);
      debugCudaSync;
    } else {
      if (temp_pop == NULL) {
      ...
      cudaCheck(cudaMemcpy(temp_pop, d_current + offp, p * n * sizeof(float), cudaMemcpyDeviceToHost));
      refDecoder.Decode(temp_pop, temp_keys);
      debugCudaSync;
      cudaCheck(cudaMemcpy(d_currFitnessKeys + offf, temp_keys, p * sizeof(float), cudaMemcpyHostToDevice));
      debugCudaSync;
    }
...
inline void GPUBRKGA<Decoder>::evolution() {
  ...
  for (int _k = 0; _k < K; _k++) {
    ...
    offspring<<<p, n>>>(d_current + offp, d_previous + offp, d_currFitnessValues + offf, d_prevFitnessValues + offf, p,
                        pe, pm, rhoe, n, d_crossStates, d_mateStates);
    debugCudaSync;

    if (gpu_deco) {
      refDecoder.Decode(d_previous + offp, d_prevFitnessKeys + offf);
      debugCudaSync;
    } else {
      if (temp_pop == NULL) {
        temp_pop = (float*)malloc(p * n * sizeof(float));
      ...
      cudaCheck(cudaMemcpy(temp_pop, d_previous + offp, p * n * sizeof(float), cudaMemcpyDeviceToHost));
      debugCudaSync;
      refDecoder.Decode(temp_pop, temp_keys);
      debugCudaSync;
      cudaCheck(cudaMemcpy(d_prevFitnessKeys + offf, temp_keys, p * sizeof(float), cudaMemcpyHostToDevice));
      debugCudaSync;
    }
    ...
    if (d_temp_storage == NULL) {
      cudaCheck(cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_prevFitnessKeys + offf,
                                                d_currFitnessKeys + offf, d_prevFitnessValues + offf,
                                                d_currFitnessValues + offf, p));
      // Allocate temporary storage
      cudaCheck(cudaMalloc(&d_temp_storage, temp_storage_bytes));
      debugCudaSync;
    }
...
template <class Decoder>
float* GPUBRKGA<Decoder>::getHostPopulations() const {
  return h_populations;
}

template <class Decoder>
float* GPUBRKGA<Decoder>::getHostFitnessKeys() const {
  return h_fitnessKeys;
}

template <class Decoder>
int* GPUBRKGA<Decoder>::getHostFitnessValues() const {
  return h_fitnessValues;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getN() const {
  return n;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getP() const {
  return p;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getPe() const {
  return pe;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getPm() const {
  return pm;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getPo() const {
  return p - pe - pm;
}

template <class Decoder>
double GPUBRKGA<Decoder>::getRhoe() const {
  return rhoe;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getK() const {
  return K;
}

template <class Decoder>
unsigned GPUBRKGA<Decoder>::getThr() const {
  return thr;
}
...
unsigned GPUBRKGA<Decoder>::getOffset(int _k, bool _pop) const {
  if (_pop)
    return _k * p * n;  // true, return chromosomes of population k offset
  else
    return _k * p;  // else return fitness of population k offset
}
